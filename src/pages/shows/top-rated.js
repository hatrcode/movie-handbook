...
const TopShows = () => {
  ...
  const url = `https://api.themoviedb.org/3/tv/top_rated?api_key=${process.env.GATSBY_TMDB_API}&include_adult=false&page=${page}&with_genres=${genreforURL}`;
  const { content, numOfPages, loading } = useFetch(url, page, selectedGenres);

  return (
    <Layout>
      <Seo title="Top Rated TV Shows" />
